...
class Game < Gosu::Window
  def initialize
    ...
    @hud = HUD.new
    @names = ["bread", "chocolate", "hot beer", "poison", "soda", "muffing", "sushi tuna", "sushi veggie"]
  ...
  def create_food_elements(names)
    ...
    names.each.with_index do |name, index|
      food = Food.new(@images[index], name, index)
      result.push(food)
    end
...
class Food
  ...
  def initialize(image, name, self_position)
    @image = image
    @width = @image.width
    @height = @image.height
    @scale = 4
    @name = name
    @x = self_position * @width * @scale
    @y = 0
  end
